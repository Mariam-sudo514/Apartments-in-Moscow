...
The prepared translations and catalog/detail discrepancies require later content review. The reservation and Home review controls perform only local validation and keep typed request drafts in memory; the Stage 8A API repeats validation and recalculates a trusted quote, but does not store or deliver booking data. The Home and reservation booking forms do not submit requests yet. Valid API requests currently return `503 DELIVERY_NOT_CONFIGURED` until a delivery workflow is implemented. CAPTCHA, email delivery, Docker/Mailpit, and legacy redirects are also outside the current scope. The client-side reservation total is preliminary and must be recalculated and validated by a future server workflow. Contacts intentionally use safe placeholders, while the legacy QR and real contact data remain excluded. The apartment gallery is implemented separately from the Home carousel.
...
Stage 8A exposes a validation-only `POST /api/booking` endpoint. It accepts strict JSON requests up to 8 KiB, requires an exact configured origin plus the `X-Booking-Request: 1` header, validates Fetch Metadata when present, and applies a fixed-window in-memory rate limiter. The documented local defaults are five requests per 60 seconds with `BOOKING_TRUST_PROXY=false`; proxy-derived client addresses are used only when explicitly enabled. Valid requests return `503 DELIVERY_NOT_CONFIGURED` with a non-PII trusted quote until server-side delivery is added. No booking data is stored, emailed, or sent by the Stage 7C review forms.

## Future work

The next stages will connect the validation-only booking API to a delivery workflow, define an anti-abuse strategy, verify email delivery through Mailpit in Docker, add legacy redirects, and complete the final SEO, accessibility, and regression review.
